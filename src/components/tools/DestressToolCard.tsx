...
const destressDuration = formatBreathworkDuration(DESTRESS_SESSION_MS)
...
export default function DestressToolCard() {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i < DESTRESS_INSTRUCTIONS.length; i += 1) {
      timeouts.push(setTimeout(() => setStepIndex(i), i * WELLNESS_STEP_MS))
    }

    timeouts.push(setTimeout(() => {
      setActivePhase('free')
    }, DESTRESS_INSTRUCTIONS.length * WELLNESS_STEP_MS))
  ...
  return (
    <BreathworkToolFocusShell focused={focused} onClose={closeSession} placeholderMinHeight={280}>
      ...
      >
        Alternate nostril breathing — follow each guided step for six counts, then breathe with the orb for five minutes. About {destressDuration}.
      </p>
            ...
            <div className="relative z-10 flex items-center justify-center px-1 min-h-[132px]">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  >
                    {DESTRESS_INSTRUCTIONS[stepIndex]}
                  </p>
                </motion.div>
              </AnimatePresence>
            </div>
