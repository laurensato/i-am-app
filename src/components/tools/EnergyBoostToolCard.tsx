...
import {
  ENERGY_BOOST_PHASES,
  ENERGY_BOOST_ROUNDS,
  ENERGY_BOOST_SESSION_MS,
  formatBreathworkDuration,
} from '@/lib/breathwork'

const energyBoostDuration = formatBreathworkDuration(ENERGY_BOOST_SESSION_MS)

type SessionState = 'idle' | 'active' | 'complete'

export default function EnergyBoostToolCard() {
  ...
  return (
    <BreathworkToolFocusShell focused={focused} onClose={closeSession} placeholderMinHeight={220}>
      ...
      >
        The 6-2 energizer — inhale deeply into your belly through your nose for 6 counts, then exhale sharply through your mouth for 2. {ENERGY_BOOST_ROUNDS} rounds, about {energyBoostDuration}.
      </p>
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full gap-3">
        <BreathworkOrb
          ...
          rounds={sessionState === 'active' ? ENERGY_BOOST_ROUNDS : undefined}
        ...
        {sessionState === 'active' && (
          <div className="flex flex-col items-center gap-1 min-h-[2.5rem]" aria-live="polite">
            ...
            <p className="text-[10px] font-light" style={{ color: 'var(--text-muted)' }}>
              Round {cycle} of {ENERGY_BOOST_ROUNDS}
            </p>
          </div>
        )}
