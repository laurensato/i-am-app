...
const wellnessDuration = formatBreathworkDuration(WELLNESS_SESSION_MS)
...
export default function WellnessToolCard() {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i < WELLNESS_INSTRUCTIONS.length; i += 1) {
      timeouts.push(setTimeout(() => setStepIndex(i), i * WELLNESS_STEP_MS))
    }

    timeouts.push(setTimeout(() => {
      setActivePhase('free')
    }, WELLNESS_INSTRUCTIONS.length * WELLNESS_STEP_MS))
  ...
  return (
    <BreathworkToolFocusShell focused={focused} onClose={closeSession} placeholderMinHeight={280}>
      ...
      >
        Diaphragmatic breathing — follow each guided step for six counts, then breathe with the orb for five minutes. About {wellnessDuration}.
      </p>
            ...
            <div className="relative z-10 flex items-center justify-center px-1 min-h-[132px]">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  >
                    {WELLNESS_INSTRUCTIONS[stepIndex]}
                  </p>
                </motion.div>
              </AnimatePresence>
            </div>
